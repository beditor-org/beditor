...
fn PanelContainer(config: PanelConfig) -> Element {
	...
	rsx! {
		if config.id == "top-bar" {
			div {
				class: "flex items-center px-4 bg-gray-900 border-b border-gray-800",
				style: "{height_style}",
				span { class: "text-sm font-medium text-gray-400", "Beditor v0.1.0" }
			}
		} else if config.id == "viewport" {
			div {
				class: "flex items-center justify-center bg-gray-950 text-gray-600",
				style: "{height_style}",
				"Game Viewport"
			}
		} else {
			div {
				...
				div {
					...
					span { class: "text-gray-300 text-sm", "{config.id}" }
				}
